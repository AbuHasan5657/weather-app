...
function renderDetailView(weatherData, cityId) {
  ...
  console.log("renderDetailView called", weatherData);
  const currentDay = forecast.forecastday[0];
  console.log("condition code:", current.condition.code);
  ...
  if (conditionImage) {
    rootElement.style.cssText = `--detail-condition-image: url(${conditionImage})`;
    rootElement.classList.add("show-background");
  }

  const isFavorite = getFavoriteCities().find(
    (city) => city.toLowerCase() === location.name.toLowerCase(),
  );
...
function getHeaderHtml(location, currentTemp, condition, maxTemp, minTemp) {
  return `
    ...
    <div class="current-weather">
        ...
        <h1 class="current-weather__temparature">${currentTemp}°</h1>
        <p class="current-weather__condition">${condition}</p>
        <div class="current-weather__day-temparatures">
          <span class="current-weather__max-temparature">H:${maxTemp}°</span>
          <span class="current-weather__min-temparature">T:${minTemp}°</span>
        </div>
    </div>
    ...
    (forecastDay, i) => `
        <div class="forecast__days">
          <div class="forecast-day">
            ...
            <div class="forecast-day__wind">Wind:${formatTemperature(forecastDay.day.maxwind_kph)} Km/h</div>
        </div>
